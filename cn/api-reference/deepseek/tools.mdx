...
# Keep the complete assistant message, including any reasoning_content and tool_calls.
...
second = client.chat.completions.create(
    model="deepseek-flash", messages=messages, tools=tools,
    tool_choice="none", reasoning_effort="low", max_tokens=1024,
)
...
first = client.responses.create(
    model="deepseek-flash", input=history, tools=tools,
...
second = client.responses.create(
    model="deepseek-flash", input=history, tools=tools,
    tool_choice="none", reasoning={"effort": "low"}, max_output_tokens=1024,
)
...
两种格式均验证单个函数、两个整数参数、显式选择函数、第二轮 `tool_choice="none"`。没有验证并行工具、Beta strict 模式、内置联网搜索，也没有把本页结论扩展为 Pro 或 Messages 的工具闭环保证。官方思考模式对历史内容有专门要求，业务中应保留完整 assistant / output 对象。[官方思考与工具历史说明](https://api-docs.deepseek.com/zh-cn/guides/thinking_mode/)
