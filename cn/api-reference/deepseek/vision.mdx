...
实测日期：**2026-09-19**。本页使用 `deepseek-flash`，通过三种协议发送同一张 PNG 参考图，三次均返回 **“A blue mug.”**，并收到正常完成标记。
...
# Download https://docs.heihuzi.ai/images/reference.png to the working directory.
encoded = base64.b64encode(Path("reference.png").read_bytes()).decode("ascii")
result = client.chat.completions.create(
    model="deepseek-flash",
    messages=[{"role": "user", "content": [
        {"type": "text", "text": "What is the main object and its color? Answer briefly in English."},
...
result = client.responses.create(
    ...
    input=[{"role": "user", "content": [
        {"type": "input_text", "text": "What is the main object and its color? Answer briefly in English."},
...
图片使用 `source.type="base64"`、`media_type="image/png"` 和不带 Data URL 前缀的 base64 数据。当前 `thinking.disabled` 不保证关闭思考；客户端只提取 text 块。
...
这证明本次渠道处理了单张 PNG 图片内容；不代表已验证远程 URL、多个图片、上传文件 ID、大图极限或所有图像格式。DeepSeek 官方将视觉输入列为 Flash 能力，Pro 不在本页视觉支持范围内。[官方视觉说明](https://api-docs.deepseek.com/zh-cn/guides/vision/)
